...
class ReportsController extends Controller
{
    ...
    /**
     * DomPDF’s bundled fonts do not include the Indian rupee sign (U+20B9), so "₹" often renders as "?".
     * "Rs." is widely used on invoices and displays correctly in PDF output.
     */
    private function pdfAmount(float|int|string|null $amount, int $decimals = 2): string
    {
    ...
    private function generateReportHTML($data, $reportType, $dateFrom, $dateTo, $outletName)
    {
        ...
        $html .= '
            <div class="footer">
                <p>Generated by DukaanTech POS System</p>
            </div>
        </body>
        </html>';
        ...
        $html = '<div class="summary">
            <div class="summary-grid">
                <div class="summary-item">
                    <h3>'.$this->pdfAmount($data['total_sales'] ?? 0).'</h3>
                    <p>Total Sales</p>
                </div>
                <div class="summary-item">
                ...
                <div class="summary-item">
                    <h3>'.$this->pdfAmount($data['average_bill_value'] ?? 0).'</h3>
    ...
    private function generateTopItemsHTML($data)
    {
        ...
        foreach ($rows as $index => $item) {
            ...
            $html .= '<tr>
                    ...
                    <td>'.$this->pdfAmount($item['total_revenue'] ?? 0).'</td>
    ...
    private function generateShiftHTML($data)
    {
        ...
        foreach ($shifts as $shift) {
                $html .= '<div class="summary" style="margin-bottom: 15px;">
                    ...
                    <div class="summary-grid">
                        <div class="summary-item">
                            <h3>'.$this->pdfAmount($shift['total_sales'] ?? 0).'</h3>
                            <p>Total Sales</p>
                        </div>
                        <div class="summary-item">
        ...
        if (isset($data['summary'])) {
            $html .= '<div class="summary">
                <div class="summary-grid">
                    ...
                    <div class="summary-item">
                        <h3>'.$this->pdfAmount($data['summary']['total_revenue'] ?? 0).'</h3>
                        <p>Total Revenue</p>
                    </div>
                    <div class="summary-item">
                        <h3>'.$this->pdfAmount($data['summary']['average_order_value'] ?? 0).'</h3>
        ...
        if (isset($data['order_types']) && is_array($data['order_types'])) {
            ...
            foreach ($data['order_types'] as $orderType) {
                $html .= '<tr>
                    ...
                    <td>'.$this->pdfAmount($orderType['total_revenue'] ?? 0).'</td>
                    <td>'.$this->pdfAmount($orderType['average_order_value'] ?? 0).'</td>
                </tr>';
            }
