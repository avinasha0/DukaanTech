...
<head>
    ...
    <title>Edit Discount - {{ $tenant->name }} - Dukaantech POS</title>
...
<body class="bg-gradient-to-br from-green-50 via-white to-teal-50 min-h-screen">
    ...
    <div class="min-h-screen py-10">
        <div class="mx-auto max-w-3xl px-4">
            ...
            <nav class="mb-6">
                <ol class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <li class="flex items-center">
                        ...
                        <span class="text-gray-900 font-medium">Edit Discount</span>
                    </li>
                </ol>
            </nav>

            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 mb-6">Edit Discount</h1>
            ...
            <form method="POST" action="{{ route('tenant.discounts.update', ['tenant' => $tenant->slug, 'discount' => $discount]) }}" x-data="discountForm()" class="bg-white shadow-xl rounded-2xl p-6 border border-gray-100 space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
                    <input name="name" type="text" required class="w-full border-gray-300 rounded-lg" value="{{ old('name', $discount->name) }}">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Code</label>
                        <input name="code" type="text" required class="w-full border-gray-300 rounded-lg" value="{{ old('code', $discount->code) }}">
                    </div>
                    <div>
                ...
                <div x-show="showValue()" x-cloak>
                    ...
                    <input name="value" type="number" step="0.01" min="0" class="w-full border-gray-300 rounded-lg" value="{{ old('value', $discount->value) }}">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-show="showBxGy()" x-cloak>
                    <div>
                        ...
                        <input name="buy_quantity" type="number" min="1" class="w-full border-gray-300 rounded-lg" value="{{ old('buy_quantity', $discount->buy_quantity) }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Get quantity</label>
                        <input name="get_quantity" type="number" min="1" class="w-full border-gray-300 rounded-lg" value="{{ old('get_quantity', $discount->get_quantity) }}">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="minimum_amount" type="number" step="0.01" min="0" class="w-full border-gray-300 rounded-lg" value="{{ old('minimum_amount', $discount->minimum_amount) }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Maximum discount</label>
                        <input name="maximum_discount" type="number" step="0.01" min="0" class="w-full border-gray-300 rounded-lg" value="{{ old('maximum_discount', $discount->maximum_discount) }}">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="valid_from" type="date" class="w-full border-gray-300 rounded-lg" value="{{ old('valid_from', optional($discount->valid_from)->format('Y-m-d')) }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Valid until</label>
                        <input name="valid_until" type="date" class="w-full border-gray-300 rounded-lg" value="{{ old('valid_until', optional($discount->valid_until)->format('Y-m-d')) }}">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="applicable_items_input" type="text" class="w-full border-gray-300 rounded-lg" placeholder="e.g. 1,2,3" value="{{ old('applicable_items_input', is_array($discount->applicable_items) ? implode(',', $discount->applicable_items) : $discount->applicable_items) }}">
                    ...
                    <div>
                        ...
                        <input name="applicable_order_types_input" type="text" class="w-full border-gray-300 rounded-lg" placeholder="e.g. dine-in,takeaway" value="{{ old('applicable_order_types_input', is_array($discount->applicable_order_types) ? implode(',', $discount->applicable_order_types) : $discount->applicable_order_types) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="usage_limit" type="number" min="1" class="w-full border-gray-300 rounded-lg" value="{{ old('usage_limit', $discount->usage_limit) }}">
                    </div>
                    <div class="flex items-center gap-3 mt-6">
                        <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded" {{ $discount->is_active ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full border-gray-300 rounded-lg">{{ old('description', $discount->description) }}</textarea>
                </div>

                <div class="pt-2 flex items-center gap-3">
                    <button type="submit" onclick="submitDisable(event)" class="inline-flex items-center px-6 py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-all">Update</button>
                    <a href="{{ route('tenant.discounts.index', ['tenant' => $tenant->slug]) }}" class="text-gray-600 hover:text-gray-800">Cancel</a>
                </div>
            </form>
        </div>
    </div>
